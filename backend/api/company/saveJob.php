...
// Validar campos
if (!isset($input['title'], $input['schedule'], $input['min_salary'], $input['max_salary'], $input['company_name'],$input['company_location'] )) {
...
try {
    ...
    $stmt = $pdo->prepare("INSERT INTO vacante (title, schedule, min_salary, max_salary, company_name, company_location) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $input['title'],
        $input['schedule'],
        $input['min_salary'],
